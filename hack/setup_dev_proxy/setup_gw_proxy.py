...
def make_blocking_ssh_call(client, cmd):
   _, stdout, _ = client.exec_command(cmd)
   print(cmd)
   print("Cmd exit status: ", stdout.channel.recv_exit_status())
...
if __name__ == "__main__":
   ...
   ssh_client, _ = create_ssh_scp_client(args.server, args.user)

   # step 2: Run cmds to install tiny proxy, append upstream rule
   # into /etc/tinyproxy.conf and finally restart the service.
   make_blocking_ssh_call(ssh_client, "rm /etc/tinyproxy.conf")
   data = conf_file_content.format(ip=args.upstream_ip, port=args.port)
   make_blocking_ssh_call(ssh_client, "cat > /etc/tinyproxy.conf << EOF\n {}\nEOF".format(data))
   make_blocking_ssh_call(ssh_client, "apt-get install tinyproxy -y")
   make_blocking_ssh_call(ssh_client, "service tinyproxy restart")
   print("Proxy is setup now.")

# Note: to remove tinyproxy and delete the conf, run following cmd on gateway: service tinyproxy stop; apt-get remove tinyproxy -y; rm /etc/tinyproxy.conf