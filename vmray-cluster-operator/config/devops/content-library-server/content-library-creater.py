...
def _make_item(
    ...
    if vcsp_type == VCSP_TYPE_OVF:
        ...
        type_metadata_value = (
            '{"id":"%s","version":"%s","libraryIdParent":"%s","isVappTemplate":"%s","vmTemplate":null,"vappTemplate":null,"networks":[],"storagePolicyGroups":null}'
            % (item_id, str(version), library_id, is_vapp_template)
        )
        type_metadata = {
            "key": "type-metadata",
...
def _dir2item(path, directory, md5_enabled, lib_id):
    ...
    for f in os.listdir(path):
        if f == ".DS_Store" or f == "".join((directory, os.extsep, FORMAT)):
            ...
            files_items.append(
                {"name": f, "size": size, "etag": folder_md5, "hrefs": [href]}
            )
    return _make_item(
        name,
        vcsp_type,
        ...
        identifier=uuid.uuid4(),
        library_id=lib_id,
        is_vapp_template=is_vapp,
    )


def _dir2item_s3(
    s3_client, bucket_name, path, item_name, skip_cert, lib_id, old_item=""
):
    """
    ...
    for content in response["Contents"]:
        ...
        last_modified = content[
            "LastModified"
        ]  # sample 'LastModified': datetime.datetime(2018, 7, 23, 16, 24, 3, tzinfo=tzutc())
        file_json = {
            "name": file_name,
            "size": size,
        ...
        if vcsp_type == VCSP_TYPE_ISO:
            ...
            items_json[child_item_name] = _make_item(
                item_path,
                vcsp_type,
                child_item_name,
                [file_json],
            ...
            if (
                vcsp_type == VCSP_TYPE_OVF
                and file_name.endswith(FILE_EXTENSION_CERT)
                and skip_cert
            ):
    ...
    if vcsp_type != VCSP_TYPE_ISO:
        ...
        items_json[item_name] = _make_item(
            item_name,
            vcsp_type,
            item_name,
            files_items,
...
def make_vcsp_s3(lib_name, lib_path, skip_cert, aws_default_region=None):
    ...
    try:
        ...
        raise Exception(
            "The give bucket %s doesn't exist or you have no access to it" % bucket_name
        )
    ...
    response = s3_client.list_objects_v2(
        Bucket=bucket_name, Prefix=lib_folder_path, Delimiter="/"
    )
            ...
            for item_path, item_json in items_json.items():
                ...
                if item_path not in old_items and updating_lib:
                    ...
                    if "contentVersion" in old_items[item_path]:
                        item_json["contentVersion"] = old_items[item_path][
                            "contentVersion"
                        ]
                    else:
                        changed = True

                    if "type-metadata" not in str(
                        old_items[item_path]
                    ) and "type-metadata" in str(item_json):
                        update_items_json = True

                    file_names = set([i["name"] for i in item_json["files"]])
                    old_file_names = set(
                        [i["name"] for i in old_items[item_path]["files"]]
                    )
                    if file_names != old_file_names:
                        # files added or removed
                        changed = True
    ...
    obj.put(
        Body=json.dumps(_make_lib(lib_name, lib_id, lib_create, lib_version), indent=2)
    )
...
def parse_options():
    ...
    # Run options
    parser.add_argument("-n", "--name", dest="name", help="library name")
    parser.add_argument(
        "-t", "--type", dest="type", default="local", help="storage type"
    )
    parser.add_argument("-path", "--path", dest="path", help="library path on storage")
    parser.add_argument("--etag", dest="etag", default="true", help="generate etag")
    parser.add_argument(
        "--skip-cert", dest="skip_cert", default="true", help="skip OVF cert"
    )
...
def usage():
    ...
    """
    return """Usage: python content-library-creator.py -n <library-name> -t <storage-type:local or s3, default local> -p <library-storage-path> --etag <true or false, default true> 
